...
describe('UI Multi-write Parsing Functions', () => {
  describe('parseCoilValues', () => {
    ...
    it('throws error for invalid coil values', () => {
      expect(() => parseCoilValues('1,2,0')).toThrow(
        'Invalid coil value: 2. Must be 0 or 1.'
      )
      expect(() => parseCoilValues('1,-1,0')).toThrow(
        'Invalid coil value: -1. Must be 0 or 1.'
      )
    })
    ...
    it('throws error for too many coils', () => {
      ...
      expect(() => parseCoilValues(tooManyCoils)).toThrow(
        'Too many coils: 1969. Maximum is 1968.'
      )
    })
  ...
  describe('parseRegisterValues', () => {
    ...
    it('throws error for invalid register values', () => {
      expect(() => parseRegisterValues('1234,abc,5678')).toThrow(
        'Invalid register value: abc. Must be 0-65535.'
      )
      expect(() => parseRegisterValues('1234,-1,5678')).toThrow(
        'Invalid register value: -1. Must be 0-65535.'
      )
      expect(() => parseRegisterValues('1234,65536,5678')).toThrow(
        'Invalid register value: 65536. Must be 0-65535.'
      )
    })

    it('throws error for empty input', () => {
      expect(() => parseRegisterValues('')).toThrow(
        'No register values provided'
      )
    ...
    it('throws error for too many registers', () => {
      ...
      expect(() => parseRegisterValues(tooManyRegisters)).toThrow(
        'Too many registers: 124. Maximum is 123.'
      )
    })
