...
describe('Frame Fuzzing Tests', () => {
        ...
        fc.property(
          ...
          (slaveId, functionCode, address, quantity) => {
            ...
            if ([1, 2, 3, 4].includes(functionCode)) {
              ...
              const value = functionCode === 5 ? (quantity % 2 ? 0xff00 : 0x0000) : quantity
              frame = [
                slaveId,
                functionCode,
                (address >> 8) & 0xff,
              ...
              const byteCount = functionCode === 15 ? Math.ceil(quantity / 8) : quantity * 2
              frame = [
                slaveId,
                functionCode,
                (address >> 8) & 0xff,
            ...
            const frameCRC = (frame[frame.length - 1] << 8) | frame[frame.length - 2]
    ...
    it('rejects frames with invalid CRC', () => {
      fc.assert(
        fc.property(
          ...
          fc.integer({ min: 0, max: 65535 }), // Corrupt CRC
      ...
      fc.assert(
        fc.property(
          ...
          (randomString) => {
            ...
            const bytes = new Uint8Array(Array.from(randomString).map(c => c.charCodeAt(0)))
    ...
    it('rejects ASCII frames with invalid LRC', () => {
      fc.assert(
        fc.property(
          ...
          fc.integer({ min: 0, max: 65535 }),
          fc.integer({ min: 1, max: 100 }),
          fc.integer({ min: 0, max: 255 }), // Wrong LRC
  ...
  describe('Buffer Boundary Testing', () => {
    ...
    it('handles various frame chunk sizes with property testing', () => {
      fc.assert(
        fc.property(
          fc.integer({ min: 2, max: 8 }), // Smaller range for more reliable testing
          fc.integer({ min: 1, max: 10 }), // Data value
          (numChunks, dataValue) => {
            const client = new ModbusClient()
            ...
            for (let i = 0; i < numChunks; i++) {
              const end = i === numChunks - 1 ? frame.length : Math.min(offset + chunkSize, frame.length)
